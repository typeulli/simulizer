...
export type WorkerOutMsg =
    | { type: "ready" }
    | { type: "log"; kind: "info" | "success" | "error"; text: string }
    | { type: "bar_create"; barId: number; min: number; max: number }
    | { type: "bar_set"; barId: number; val: number }
    | { type: "visual"; data: number[]; rows: number; cols: number }
...
self.onmessage = async (e: MessageEvent<WorkerInMsg>) => {
    ...
    try {
        ...
        const imports: WebAssembly.Imports = {
            debug: {
                log: (val: number) => post({ type: "log", kind: "info", text: `🔍 log: ${val}` }),
                log_ptr: (ptr: number) => post({ type: "log", kind: "info", text: `🔍 ptr: 0x${ptr.toString(16)}` }),
                log_arr_i32: (ptr: number, cap: number) => {
                    if (!wasmMemory) return;
                    const arr = simulizer.pipe.load_arr_i32(wasmMemory, ptr, cap);
                    post({ type: "log", kind: "info", text: `📚 [${arr?.join(", ") ?? "undefined"}]` });
                },
            ...
            tensor: {
                ...
                show_mat: (tensorId: number) => {
                    ...
                    if (field) {
                        post({ type: "visual_vec", dx: Array.from(field.dx), dy: Array.from(field.dy), rows: field.rows, cols: field.cols });
                        return 0;
                    }
                    const mat = js_tensor_get_mat_data(tensorId);
                    if (mat) post({ type: "visual", data: Array.from(mat.data), rows: mat.rows, cols: mat.cols });
                    return 0;
                },
